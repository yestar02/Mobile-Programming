...
class TaskNoteViewModel : ViewModel() {
    ...
    fun addTaskItem() {
        ...
        if (dueDate.isBlank())
            itemList.add(TaskNoteType.Task(id = nextId(), title = title))
        else
            itemList.add(TaskNoteType.Task(id = nextId(), title = title, dueDate = dueDate))
    ...
    private fun nextId(): Int =
        (itemList.maxOfOrNull { it.id } ?: -1) + 1
